...
function Product() {
  ...
  const handleAdd = async () => {
    ...
    try {
      await axios.post(`${backendUrl}/index/productadd`, formData, {
        headers: {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await axios.get(`${backendUrl}/index/getproduct`);
  ...
  const handleDelete = async () => {
    try {
      if (id) {
        await axios.delete(`${backendUrl}/index/productDelete/${id}`);
  ...
  const handleEdit = async () => {
    ...
    try {
      await axios.put(`${backendUrl}/index/editProduct/${id}`, formData, {
        "Content-type": "multipart/form-data",
      });
  ...
  const handleeditchange =async(e,id)=>{
      ...
      try{
       await axios.put(`${backendUrl}/index/editstatus/${id}`,datas)
       window.location.href = "/product?message=Status%20updated%20successfully";
      }catch(err){
        console.log(err);
      }
  }
            ...
            {list.map((item, index) => {
              return (
                <tr key={index}>
                  ...
                  ></td>
                   <td className="text-left" style={{ fontSize: "15px" }}>
                   <select  style={{ width: "100%", height: "40px", fontSize: "14px" }}  onChange={(e) => handleeditchange(e, item._id)}>
      ...
      <div>
        <Dialog maxWidth="sm" fullWidth open={open}>
          ...
          <DialogContent>
            ...
            <select  style={{ width: "100%", height: "40px", fontSize: "14px" }} onChange={(e)=> setStatus(e.target.value)}>
